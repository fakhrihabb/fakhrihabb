      ...
      <div className="container mx-auto max-w-7xl w-full">
        ...
        <div className="grid md:grid-cols-3 gap-8 items-stretch">
          {wins.slice().reverse().map((win, index) => {
            ...
            const isLatestWin = index === 0;
            const remainder = totalWins % 3;

            // Determine column span for latest win
            let colSpan = '';
            let imageHeight = 'h-64'; // default height
            let isHorizontal = false;

            if (isLatestWin && remainder === 2) {
              colSpan = 'md:col-span-2';
              imageHeight = 'h-80'; // taller for 2-column span
            } else if (isLatestWin && remainder === 1) {
              colSpan = 'md:col-span-3';
              imageHeight = 'h-full'; // full height for horizontal layout
              isHorizontal = true;
            }
            ...
            return (
              <div
                ...
                className={`group glass glass-hover rounded-2xl overflow-hidden relative ${isHorizontal ? 'flex flex-col md:flex-row' : 'flex flex-col'} h-full ${colSpan}`}
                style={{ transform: 'none' }}
              >
                {/* Win Image */}
                <div className={`relative ${isHorizontal ? 'md:w-2/5' : 'w-full'} ${imageHeight} ${!isHorizontal ? 'overflow-hidden' : 'md:min-h-full overflow-hidden'}`}>
                  <Image
                    src={win.image}
                    alt={win.competition}
                    fill
                  ...
                  {isLatestWin && (
                    <div className="absolute top-4 right-4 z-10">
                      <span className="px-3 py-1.5 bg-brand-primary/90 backdrop-blur-sm border-2 border-brand-primary text-sm font-bold text-bg-primary uppercase tracking-wider rounded-lg shadow-lg" style={{ fontFamily: 'var(--font-vt323), monospace' }}>
                        ⭐ Latest
                      </span>
                    </div>
                  )}
                ...
                {/* Content */}
                <div className={`p-6 space-y-4 flex flex-col ${isHorizontal ? 'md:flex-1 justify-center' : 'flex-grow'}`}>

                  {/* Rank Badge */}
                  <div className={`flex ${isHorizontal ? 'md:justify-start justify-center' : 'justify-center'}`}>
                  ...
                  {/* Competition Name */}
                  <h3 className={`text-2xl font-bold text-text-primary ${isHorizontal ? 'md:text-left text-center' : 'text-center'} group-hover:text-brand-primary transition-colors duration-300`}>
                    {win.competition}
                  </h3>

                  {/* Organizer */}
                  <p className={`text-lg text-text-secondary ${isHorizontal ? 'md:text-left text-center' : 'text-center'}`} style={{ fontFamily: 'var(--font-vt323), monospace' }}>
                    {win.organizer}
                  </p>

                  {/* Date */}
                  <p className={`text-lg text-text-tertiary ${isHorizontal ? 'md:text-left text-center' : 'text-center'}`} style={{ fontFamily: 'var(--font-vt323), monospace' }}>
                    {win.date}
                  </p>

                  {/* Description */}
                  <p className={`text-xl text-text-secondary leading-relaxed ${isHorizontal ? 'md:text-left text-center' : 'text-center'} pt-4 flex-grow`} style={{ fontFamily: 'var(--font-vt323), monospace' }}>
                    {win.description}
                  </p>

                  {/* Technologies/Categories */}
                  {win.categories && (
                    <div className={`flex flex-wrap gap-2 ${isHorizontal ? 'md:justify-start justify-center' : 'justify-center'} pt-4`}>
                  ...
                  {/* Link */}
                  {win.link && win.link !== "#" && (
                    <div className={`flex ${isHorizontal ? 'md:justify-start justify-center' : 'justify-center'} pt-4`}>
                      <a
                        href={win.link}
